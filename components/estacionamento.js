...
export default function ToDoList() {
  ...
  async function fetchTodos(completed) {
    let path = '/estacionamento/all'
    if (completed !== undefined) {
      path = `/all?completed=${completed}`
    }
  ...
  async function addToDo(placaVeiculo) {
    const res = await fetch(process.env.NEXT_PUBLIC_API_URL + `/estacionamento/registrar`, {
      method: 'POST',
      body: JSON.stringify({
  ...
  return (
    <div className={styles.container}>
      <div className={styles.mainInputContainer}>
        <input className={styles.mainInput} placeholder="Placa veículo" value={mainInput} onChange={(e) => handleMainInputChange(e)} onKeyDown={handleKeyDown}></input>
      </div>
      {!todos && (
        <div>Loading...</div>
      )}
